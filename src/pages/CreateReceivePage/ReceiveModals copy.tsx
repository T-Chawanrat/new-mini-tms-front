...
export default function ReceiveModals({
  ...
  const selectedPackageHasDetails = selectedPackageDetails.length > 0;
  const selectedPackageIsWeightFix = selectedPackageHasDetails && selectedPackageDetails.every((item) => isWeightFix(item));
  const selectedPackageIsRateCalculate = Boolean(packageForm.package_id && selectedPackageHasDetails && !selectedPackageIsWeightFix);

  const selectedPackageLimits = useMemo(() => {
    if (!selectedPackageDetails.length) {
      return { maxSize: 0, maxWeight: 0 };
    }

    return selectedPackageDetails.reduce(
      (max, item) => ({
        maxSize: Math.max(max.maxSize, toNum(item.size_max)),
        maxWeight: Math.max(max.maxWeight, toNum(item.weight_max)),
      }),
      { maxSize: 0, maxWeight: 0 },
    );
  }, [selectedPackageDetails]);

  const packageValueTooHigh =
    selectedPackageIsRateCalculate &&
    ((selectedPackageLimits.maxSize > 0 && toNum(packageForm.q) > selectedPackageLimits.maxSize) ||
      (selectedPackageLimits.maxWeight > 0 && toNum(packageForm.weight) > selectedPackageLimits.maxWeight));

  const packageBaseReady = Boolean(
    packageForm.package_id &&
    packageForm.package_name.trim() &&
    packageForm.package_detail_id &&
    packageForm.package_size_name.trim() &&
    toNum(packageForm.unit_price) > 0,
  );

  const packageDimensionReady =
    !selectedPackageIsRateCalculate ||
    (toNum(packageForm.width) > 0 &&
      toNum(packageForm.length) > 0 &&
      toNum(packageForm.height) > 0 &&
      toNum(packageForm.q) > 0 &&
      toNum(packageForm.weight) > 0);

  const canSavePackage = Boolean(packageBaseReady && packageDimensionReady && !packageValueTooHigh && toNum(packageForm.qty) > 0);
  const canSaveScan = Boolean(packageBaseReady && packageDimensionReady && !packageValueTooHigh && scanBarcode.trim());

  const clearCalculatedRate = () => {
  ...
  const updateCalculatedPackageForm = (field: "width" | "length" | "height" | "weight", value: string) => {
    if (!selectedPackageIsRateCalculate) return;

    if (value === "") {
      setPackageError(null);
      updatePackageForm(field, "");
    ...
    const inputNumber = toNum(value);

    if (inputNumber < 0) return;

    if (field === "weight" && selectedPackageLimits.maxWeight > 0 && inputNumber > selectedPackageLimits.maxWeight) {
      setPackageError(`น้ำหนักห้ามเกิน ${moneyValue(selectedPackageLimits.maxWeight)} ก.ก.`);
      clearCalculatedRate();
      return;
    }
    ...
    const width = toNum(nextForm.width);
    const length = toNum(nextForm.length);
    const height = toNum(nextForm.height);
    const weight = toNum(nextForm.weight);
    const q = width > 0 && length > 0 && height > 0 ? width * length * height : 0;

    if (q > 0 && selectedPackageLimits.maxSize > 0 && q > selectedPackageLimits.maxSize) {
      setPackageError(`Q ห้ามเกิน ${moneyValue(selectedPackageLimits.maxSize)} ลบ.ซม.`);
      clearCalculatedRate();
      return;
    }
    ...
    updatePackageForm("q", q > 0 ? moneyValue(q) : "");
    ...
    const rate = calculatePackageRate(selectedPackageDetails, q, weight);

    if (!rate) {
      clearCalculatedRate();
      setPackageError("ไม่พบเรทราคาที่ตรงกับ Q และน้ำหนักที่กรอก");
      return;
    }

    updatePackageForm("package_detail_id", rate.package_detail_id != null ? String(rate.package_detail_id) : "");
    updatePackageForm("package_detail_code", rate.package_detail_code || "");
    updatePackageForm("package_detail_type", rate.package_detail_type || "");
    updatePackageForm("package_size_name", rate.package_detail_name || "");
    updatePackageForm("unit_price", moneyValue(rate.cost));
  };

  const formatCalculatedPackageField = (field: "width" | "length" | "height" | "weight") => {
    const value = toNum(packageForm[field]);
    if (value <= 0) return;
    updateCalculatedPackageForm(field, moneyValue(value));
  };
  ...
  const selectPackageDetail = (detailId: string) => {
    ...
    updatePackageForm("q", detail.size_max != null ? moneyValue(detail.size_max) : "");
    updatePackageForm("weight", detail.weight_max != null ? moneyValue(detail.weight_max) : "");
    updatePackageForm("unit_price", moneyValue(detail.cost));
  };
          ...
          <div className="w-full max-w-[680px] rounded-md bg-white p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
                ...
                <div className="space-y-2">
                  <div className="grid grid-cols-[96px_1fr] items-center gap-2">
                    ...
                    <input
                      ...
                      value={packageForm.package_code ? `${packageForm.package_code} - ${packageForm.package_name}` : packageForm.package_name}
                      readOnly
                  ...
                  <div className="grid grid-cols-[96px_1fr] items-center gap-2">
                    ...
                    <select
                      ...
                      disabled={!packageForm.package_id || selectedPackageIsRateCalculate}
                    >
                      <option value="">{selectedPackageIsRateCalculate ? "ระบบจะคำนวณราคาให้" : "เลือกรายละเอียด"}</option>
              ...
              <div className="rounded-md border border-slate-200 bg-slate-50/80 p-2.5">
                <div className="mb-2 flex items-center justify-between gap-2">
                  ...
                  {selectedPackageIsRateCalculate && (
                    <div className="text-[10px] text-amber-700">
                      ไม่เกิน{" "}
                      {selectedPackageLimits.maxSize || selectedPackageLimits.maxWeight
                        ? `(${[
                            selectedPackageLimits.maxSize ? `${moneyValue(selectedPackageLimits.maxSize)} ลบ.ซม.` : "",
                            selectedPackageLimits.maxWeight ? `${moneyValue(selectedPackageLimits.maxWeight)} ก.ก.` : "",
                          ]
                            .filter(Boolean)
                            .join(" / ")})`
                        : ""}
                ...
                <div className="grid grid-cols-3 gap-2 md:grid-cols-6">
                  <div>
                    ...
                    <input
                      ...
                      readOnly={!selectedPackageIsRateCalculate}
                      onChange={(e) => updateCalculatedPackageForm("width", e.target.value)}
                      onBlur={() => formatCalculatedPackageField("width")}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      readOnly={!selectedPackageIsRateCalculate}
                      onChange={(e) => updateCalculatedPackageForm("length", e.target.value)}
                      onBlur={() => formatCalculatedPackageField("length")}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      readOnly={!selectedPackageIsRateCalculate}
                      onChange={(e) => updateCalculatedPackageForm("height", e.target.value)}
                      onBlur={() => formatCalculatedPackageField("height")}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      type="number"
                      min="0"
                      max={selectedPackageLimits.maxWeight || undefined}
                      value={packageForm.weight}
                      readOnly={!selectedPackageIsRateCalculate}
                      onChange={(e) => updateCalculatedPackageForm("weight", e.target.value)}
                      onBlur={() => formatCalculatedPackageField("weight")}
                    />
                  </div>
                ...
                {selectedPackageIsRateCalculate && packageForm.package_size_name && (
                  <div className="mt-2 rounded border border-blue-100 bg-blue-50 px-2 py-1 text-[10px] text-blue-700">
                    เรทราคาที่ระบบเลือก: {packageForm.package_size_name}
                  </div>
                )}
              </div>
            ...
            <div className="mt-4 flex justify-end">
              <button
                ...
                className={`rounded px-5 py-1.5 text-xs font-semibold text-white ${
                  canSavePackage ? "bg-blue-600 hover:bg-blue-700" : "cursor-not-allowed bg-slate-300"
                }`}
              >
                บันทึก
              </button>
              ...
              <table className="w-full border-collapse text-xs">
                ...
                <tbody>
                  {packageGroups.map((item) => {
                    const itemIsWeightFix = item.details.length > 0 && item.details.every((detail) => isWeightFix(detail));
                          ...
                          <div className="mt-0.5 flex flex-wrap items-center gap-1.5 text-[10px] text-slate-500">
                            <span
                              className={`inline-flex rounded-full border px-2 py-0.5 text-[10px] font-semibold ${
                                itemIsWeightFix ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-amber-200 bg-amber-50 text-amber-700"
                              }`}
                            >
                              {itemIsWeightFix ? "Fix" : "Q"}
                            </span>
                            <span>{item.details.length ? `${item.details.length} detail` : "ไม่มี detail"}</span>
                          </div>
                        </td>
      ...
      {showScanModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4">
          <div className="w-full max-w-[680px] rounded-md bg-white p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="mb-4 flex items-start justify-between">
              <div>
                <h3 className="text-base font-semibold text-slate-900">สแกนบาร์โค้ด</h3>
              </div>
            ...
            <div className="mt-4 flex justify-end">
              <button
                ...
                className={`rounded px-5 py-1.5 text-xs font-semibold text-white ${
                  canSaveScan ? "bg-blue-600 hover:bg-blue-700" : "cursor-not-allowed bg-slate-300"
                }`}
              >
                บันทึก
              </button>
