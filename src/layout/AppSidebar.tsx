        ...
        <li key={nav.name} className="relative">
          {nav.subItems ? (
            <>
              ...
              {nav.subItems && !isExpanded && !isMobileOpen && openSubmenu?.type === menuType && openSubmenu?.index === index && (
                  ...
                  {nav.subItems
                    ...
                    .map((subItem) => (
                      <Link
                        ...
                        title={subItem.name}
                      >
                        <span className={`${isActive(subItem.path) ? "text-brand-500" : "text-gray-600"}`}>{subItem.icon}</span>
                      </Link>
                    ))}
                </div>
              )}
        ...
        <nav className="mb-6">
          <div className="flex flex-col gap-4">
            <div>
              {user && (
                <>
                  <h2 className={`mb-4 uppercase flex leading-[20px] text-slate-400 ${!isExpanded ? "lg:justify-center" : "justify-start"}`}>
        ...
        {user && (
          ...
          "
            >
              ...
              {(isExpanded || isMobileOpen) && <span className="menu-item-text text-brand-500">Logout</span>}
            </button>
          </div>
        )}
      </div>
